(() => {
  ...
  async function requestPreview(snapshot, seed) {
    ...
    const preview = previewFromResult(result, snapshot, seed);
    if (!preview.pending) return preview;
    currentDraft.preview = preview; currentDraft.stale = false; state.pendingMusic = { preview, snapshot, seed }; saveState(); updateStudioMeta();
    const completed = await waitForPreview(preview, snapshot, seed);
  ...
  async function generatePreview() {
    ...
    if (!["remote", "mock"].includes(musicServiceMeta.mode)) { showToast("专业音乐服务尚未配置，暂时不能生成真唱试听", "error"); return false; }
  ...
  function syncPlayerProgress() {
    const current = Number(globalPlayerAudio?.currentTime) || 0; const duration = activePlaybackDuration(); const active = Boolean(globalPlayerAudio && globalPlayerSong);
  ...
  function updateGlobalPlayer() {
    if (!globalPlayerAudio || !globalPlayerSong) { syncPlayerProgress(); return; }
    const time = Number(globalPlayerAudio.currentTime) || 0;
    let active = -1;
  ...
  function syncGlobalPlayerButtons() {
    const playing = Boolean(globalPlayerAudio && globalPlayerPlaying);
  ...
  async function playNextCollectionSong() {
    ...
    if (collectionPlayMode === "repeat-one" && !collectionFailedIds.has(globalPlayerSong.id)) { globalPlayerAudio.currentTime = 0; try { await globalPlayerAudio.play(); globalPlayerPlaying = true; } catch (_) { return false; } syncGlobalPlayerButtons(); updateGlobalPlayer(); return true; }
  ...
  function renderSongDetail() {
    ...
    $("#songDetailMeta").textContent = `${styleNames[song.style] || song.style} · ${formatDate(song.createdAt)} · ${musicProviderName(song.provider)} 真唱${song.alignment ? " · 真实歌词时间轴" : " · 估算逐行同步"}`;
